<template>
	<view style="padding-top: 10px;">
		...
		<view class="content" style="padding: 10px;">
			<view v-if="current === 0" v-for="(item,index) in allOrder" :key="index" @click="goToOrderDetails(index)">
	...
	export default {
		...
		methods: {
			...
			onClickItem(e) {
				...
				if(e.currentIndex==0) {
					this.nowOrder = this.allOrder
				}else if(e.currentIndex==1) {
					this.nowOrder = this.allOrder.filter(item=>item.orderInfo.status=='待支付')
				}
